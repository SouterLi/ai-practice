...
# -----------------------------------------------------------------------------
# 日志与 trace_id（每个用户请求在进入 Agent 前设置）
# -----------------------------------------------------------------------------
trace_id_ctx: ContextVar[str] = ContextVar("trace_id", default="")


class _TraceIdFilter(logging.Filter):
    """为每条日志补齐 trace_id 字段（供 Formatter 使用）。"""

    def filter(self, record: logging.LogRecord) -> bool:
        setattr(record, "trace_id", trace_id_ctx.get() or "-")
        return True


def _setup_logging(level: int = logging.INFO) -> None:
    # 只配置本模块 logger，避免清空 root 影响其它库
...
class TraceAgentCallbackHandler(BaseCallbackHandler):
    """打关键链路与工具事件，便于与业务 trace_id 对齐排查。"""
    ...
    def on_tool_start(
        ...
        **kwargs: Any,
        ...
        logger.info("工具启动 name=%s run_id=%s input=%s", name, run_id, input_str)

    def on_tool_end(self, output: Any, *, run_id: Any, **kwargs: Any) -> None:
        out = getattr(output, "content", output)
        preview = (out[:500] + "…") if isinstance(out, str) and len(out) > 500 else out
        logger.info("工具结束 run_id=%s output_preview=%s", run_id, preview)

    def on_tool_error(self, error: BaseException, *, run_id: Any, **kwargs: Any) -> None:
        logger.error("工具报错 run_id=%s err=%s", run_id, error, exc_info=error)
...
def build_agent_executor() -> AgentExecutor:
    ...
    callbacks = cast(list[BaseCallbackHandler], [TraceAgentCallbackHandler()])
